document.addEventListener("DOMContentLoaded", function () {
    var felizElement = document.querySelector(".feliz");

    felizElement.addEventListener("animationend", function () {
        initializeFireworks();
    });
});
...
function Firework(sx, sy, tx, ty) {
    ...
    this.speed = 2;
    this.acceleration = 1.03; //1.05 default
...
var limiterTotal = 90; // Ändere die Anzahl der automatisch abgefeuerten Feuerwerke
...
function update() {
    ...
    if (timerTick >= timerTotal) {
        if (!mousedown) {
            // Erzeuge mehr automatische Feuerwerke
            var autoFireworksCount = 5;
            while (autoFireworksCount--) {
    ...
    if (limiterTick >= limiterTotal) {
        if (mousedown) {
            // Erzeuge mehr Feuerwerke beim Klicken
            var clickFireworksCount = 5;
            while (clickFireworksCount--) {
